...
class InvoiceExtractor:
    ...
    def preprocess_image(pil_image):
        """Convert PIL image to grayscale + adaptive threshold for OCR clarity."""
    ...
    def find_label_bbox(self, results, label_variants):
        """
        Find bounding boxes of any label variant in OCR results.
        Returns list of tuples (bbox, original_text).
        """
        found = []
    ...
    def find_values_near_label_multiple(self, results, label_bbox, max_x_dist=350, max_y_dist=60):
        """Return all text near the label bbox within spatial thresholds."""
    ...
    def find_value_near_label(self, results, label_bbox, max_x_dist=250, max_y_dist=40):
        """
        Find the closest text to the right of the label bbox horizontally aligned.
        """
        x_min_l, y_min_l, x_max_l, y_max_l = label_bbox
        label_y_center = (y_min_l + y_max_l) / 2
        candidates = []
    ...
    def extract_invoice_number(self, results):
        ...
        if not labels:
            ...
            print("Fallback all OCR texts:", all_texts)
            for text in all_texts:
                candidate = re.sub(r'\s+', '', text)
                if re.match(r'^[A-Za-z0-9\-\/]{4,20}$', candidate):
                    print(f"Fallback candidate accepted: {candidate}")
                    return candidate
            return "Not found"
        ...
        print("Candidates near label:", value_candidates)
        if value_candidates:
...
iface = gr.Interface(
    ...
    description="Upload invoice images to extract Invoice Number, Date, and Total Amount robustly."
)
